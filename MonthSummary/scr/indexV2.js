...
$(function() {
    inputSession();
    var now = new Date();    
    $("#month").val(now.getFullYear() + "-" + getTwoDigits(now.getMonth()+ 1));

    var month =new Date($("#month").val());
    var MonthLastDate = new Date(month.getFullYear(), month.getMonth() + 1, 0);
    var MonthFirstDate = new Date(month.getFullYear(), (month.getMonth() + 12) % 12, 1);

    renderHead($('div#table'), MonthFirstDate, MonthLastDate);
});
...
function makeSummaryTable() {
    ...
    var MonthLastDate = new Date(month.getFullYear(), month.getMonth() + 1, 0);
    var MonthFirstDate = new Date(month.getFullYear(), (month.getMonth() + 12) % 12, 1);
    sendObj["start_s"] = formatDate(MonthFirstDate);
    sendObj["end_s"] = formatDate(MonthLastDate);
